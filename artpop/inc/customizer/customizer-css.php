...
function artpop_custom_css() {
	...
	$site_logo_mobile_size   = absint( get_theme_mod( 'site_logo_mobile_size', artpop_defaults( 'site_logo_mobile_size') ) );
	$hide_tagline_mobile     = get_theme_mod( 'site_logo_hide_tagline_on_mobile', artpop_defaults( 'site_logo_hide_tagline_on_mobile' ) );
	...
	// Site Logo Size
	if ( ! empty( $site_logo_mobile_size ) ) {
		$theme_css .= ".mobile-header .site-logo {max-width: {$site_logo_mobile_size}px;}";
	}
	...
	if ( ! empty( $hide_tagline_mobile ) ) {
		$theme_css .= "@media screen and (max-width: 719px) { .site-description {display: none;} }";
	}
	// Colors
