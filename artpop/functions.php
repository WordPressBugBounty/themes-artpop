...
add_action( 'init', function() {
	load_theme_textdomain( 'artpop', get_template_directory() . '/languages' );
} );
...
function artpop_setup() {
	...
	add_editor_style( array( 'inc/css/editor-style.css', artpop_fonts_url() ) );
 ...
 */
function artpop_resource_hints( $urls, $relation_type ) {
	if ( wp_style_is( 'artpop-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
		$urls[] = array(
...
add_filter( 'wp_resource_hints', 'artpop_resource_hints', 10, 2 );
...
function artpop_scripts() {
	...
	if( ! artpop_is_amp() ) {
		// Main js.
		wp_enqueue_script( 'artpop-script', get_template_directory_uri() . '/assets/js/main.js', array(), '20220905', true );
...
function artpop_add_sub_menu_toggles( $output, $item, $depth, $args ) {
	if( ! artpop_is_amp() ) {
		...
		if ( isset( $args->show_sub_menu_toggles ) && $args->show_sub_menu_toggles && in_array( 'menu-item-has-children', $item->classes, true ) ) {
			$output = '<div data-amp-bind-class="visible' . $item->ID . ' ? \'menu-item-wrapper is-open\' : \'menu-item-wrapper is-closed\'"  class="menu-item-wrapper">'.  $output . '<button on="tap:AMP.setState({visible' . $item->ID . ': !visible' . $item->ID . '})" class="sub-menu-toggle" aria-expanded="false"><span class="screen-reader-text">' . __( 'Show sub menu', 'artpop' ) . '</span>' . artpop_get_svg( array( 'icon' => 'chevron-down' )  ) . '</button></div>';
		}
	}
	return $output;
}
 ...
 * Add About Page
 */
require_once get_template_directory() . '/inc/about-page/about-artpop.php';


/**
 * Add Upsell notice
 */
function artpop_notice() {
	$user_id = get_current_user_id();
	if ( ! get_user_meta( $user_id, 'artpop_notice_dismissed' ) ) {
	?>
	<div class="updated notice notice-success is-dismissible artpop-admin-notice">
		<h2 class="welcome-title">
			<?php esc_html_e( 'Welcome! Thank you for choosing Artpop WordPress Theme', 'artpop' ); ?>
		</h2>
		<p>
			<?php echo wp_kses_post( __( '<strong>To fully take advantage</strong> of the best our theme can offer, please visit our', 'artpop' ) ); ?> <a href="<?php echo esc_url( admin_url( 'themes.php?page=about_artpop' ) ); ?>"><strong><?php echo esc_html__( 'Welcome Page', 'artpop' ); ?></strong></a>
		</p>
		<p>
